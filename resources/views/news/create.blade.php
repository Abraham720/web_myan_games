...
@section('title', 'Post News • MyanGames')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">🗞️ Post Berita Baru</h1>
        <p class="text-slate-500 dark:text-gray-400 text-sm mt-1">Bagikan berita gaming terbaru dengan komunitas</p>
    </div>
    ...
    <div class="bg-white dark:bg-card rounded-xl border border-slate-200 dark:border-gray-700 p-6 shadow-xl">
        <form action="{{ route('news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="game_title" value="{{ old('game_title') }}" placeholder="Contoh: Genshin Impact 3.0 Release..."
            ...
            <div>
                ...
                <textarea name="caption" rows="5" placeholder="Tulis berita, update, atau pengumuman penting..."
            ...
            <!-- Image Upload -->
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-2">Gambar Berita <span class="text-red-400">*</span></label>
                <div class="border-2 border-dashed border-slate-300 dark:border-gray-600 rounded-lg p-6 text-center hover:border-primary/50 transition cursor-pointer group bg-slate-50 dark:bg-dark/50"
                     onclick="document.getElementById('news_image_input').click()">
                    <input type="file" name="image" id="news_image_input" accept="image/*" class="hidden" required onchange="previewNewsImage(this)">
                    <div id="news_upload_placeholder">
                        ...
                        <p class="text-red-400 text-xs mt-1 font-medium">⚠️ Gambar WAJIB untuk News</p>
                        <p class="text-slate-400 dark:text-gray-500 text-xs mt-1">PNG, JPG, GIF • Max 5MB</p>
                    </div>
                    <img id="news_image_preview" class="hidden max-h-48 mx-auto rounded-lg object-cover">
                </div>
            ...
            <div class="pt-4 border-t border-slate-200 dark:border-gray-700">
                <button type="submit" class="w-full bg-gradient-to-r from-accent to-primary hover:from-cyan-500 hover:to-indigo-600 text-white font-semibold py-3 px-6 rounded-lg transition shadow-lg shadow-accent/25 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98]">
